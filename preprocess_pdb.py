...
def main(main_folder, processed_dir):
    ...
    decompression_process = Process(
        target=worker_decompression, args=(input_queue, checker_queue)
    )
    checker_process = Process(
        target=worker_checker, args=(checker_queue, deletion_queue)
    )
    deletion_process = Process(
        target=worker_deletion, args=(deletion_queue, moving_queue)
    )
    moving_process = Process(
        target=worker_moving, args=(moving_queue, removal_queue, processed_dir)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "folder", type=str, help="Path to the folder containing PDB folders"
    )
    parser.add_argument(
        "processed_folder",
        type=str,
        help="Path to the folder where processed data will be",
    )
    parser.add_argument(
        "--log_file", type=str, default="process.log", help="Path to the log file"
    )
